...
const { data: session } = useSession();
  ...
  const storedUser = getUserFromStorage();
    ...
    fullName: session?.user?.name || storedUser?.name || "User",
    username:  session?.user?.email?.split('@')[0] || storedUser?.email?.split('@')[0] || "user",
  ...
  const infoItems = [
    { label: "Full sName", value: profile.fullName, icon: "👤", color: "#2c5364" },
